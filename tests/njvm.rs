...
fn test_stdin_output(arg: &str, stdin: &str, expected_output: &str) -> Result<()> {
    let mut cmd = Command::cargo_bin(crate_name!())?;
    let output = cmd.arg(arg).write_stdin(stdin).output()?;
    let status = output.status;
    let stdout = String::from_utf8(output.stdout)?;
    assert!(status.success(), "Command failed with exit code: {:?}", status.code());
    assert!(stdout.contains(expected_output), "Expected output not found in stdout");
    Ok(())
}
